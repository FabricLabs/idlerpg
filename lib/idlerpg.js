...
IdleRPG.prototype._handleTransferRequest = async function (message) {
  ...
  let target = await rpg._getProfile(parts[2]);
  ...
  try {
    let ops = [
      { op: 'replace', path: `/${actor.id}/wealth`, value: parseInt(actor.wealth - amount)},
      { op: 'replace', path: `/${target.id}/wealth`, value: parseInt(target.wealth + amount)}
    ];

    manager.applyPatch(this.state, ops);
  } catch (E) {
    console.error('[IDLERPG]', 'could not serialize transaction:', E);
  }

  let response = `balance transferring...`;
...
IdleRPG.prototype._getPlayers = async function () {
  ...
  for (let name in rpg.fabric.services) {
    ...
    for (let i in members) {
      ...
      let profile = Object.assign({}, remote, { id, presence });
