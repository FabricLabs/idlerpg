...
IdleRPG.prototype.start = async function () {
  ...
  // TODO: document & test
  rpg.newsletter = setInterval(function () {
    // TODO: output top 10 leaderboard
    rpg.announce(`A rooster crows in the distance, signalling the break of dawn.`);
  }, 24 * 60 * 60 * 1000);
...
IdleRPG.prototype._handleLeaderboardRequest = async function () {
  ...
  let members = list.map(x => {
    return `1. ${x.name}, with **${x.wealth}** wealth`;
  }).slice(0, 10);
...
IdleRPG.prototype._getActivePlayers = async function () {
  ...
  return players.filter(x => (x.presence === 'online')).filter(function (x) {
