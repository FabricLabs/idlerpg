...
IdleRPG.prototype.start = async function () {
  ...
  try {
    ...
    rpg.state = JSON.parse(state);
  } catch (E) {
    return console.error('Could not restore state:', E);
  }
...
IdleRPG.prototype._computeRound = async function computeRound (player) {
  ...
  // relax the cooldown...
  if (profile.cooldown) {
    profile.cooldown = profile.cooldown - PER_TICK_CAPITAL;
  }
...
IdleRPG.prototype._handleProfileRequest = async function (message) {
  ...
  let response = `You are level **${entity.level}** (having earned **${profile.experience}** experience), with **${profile.stamina}** stamina, **${profile.health}** health, and **${profile.wealth}** wealth.`;
...
IdleRPG.prototype._handleTransferRequest = async function (message) {
  ...
  if (parseInt(actor.wealth - amount) < 0) return `You do not have that amount.  You'll need **${parseInt(actor.wealth - amount)}** more Idlecoin to proceed with this transfer.`;
...
IdleRPG.prototype._getPlayers = async function () {
  ...
  for (let name in rpg.fabric.services) {
    ...
    for (let i in members) {
      let path = [name, 'users', members[i]].join('/');
      let profile = await rpg._getProfile(path);
      let player = await rpg._registerPlayer(profile);

      player.presence = await service._getPresence(path);
...
IdleRPG.prototype._registerPlayer = async function registerPlayer (player) {
  ...
  let user = await rpg.fabric.services[parts[0]]._getUser(parts[2]);
  let account = Object.assign({}, user, { id });

  await rpg._registerUser(account);
...
IdleRPG.prototype._registerChannel = async function registerChannel (channel) {
  ...
  if (!channel.name) return console.error('Channel must have a "name" property.');
