...
IdleRPG.prototype.tick = async function () {
  ...
  this.emit('tick');
  console.log(rpg.state)

  return rpg;
};

/**
  * Start of stats declaration.

IdleRPG.prototype.statistics = async function (player)
{
  let attack = Entity.attack;
  let agility = Entity.agility;
  let vitality = Entity.vitality;
  let invocation = Entity.invocation;
  let influence = Entity.influence;
  let shielding = Entity.shielding;
  let smarts = Entity.smarts;
  let willpower = Entity.willpower;
  let charm = Entity.charm;
